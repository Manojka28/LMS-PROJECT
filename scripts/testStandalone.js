...
// Wait, index.js binds to PORT. If PORT=5001, it will bind to 5001.
...
async function testPayment() {
  try {
    ...
    const instLoginRes = await fetch('http://localhost:5001/api/auth/login', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
    ...
    const initInstRes = await fetch('http://localhost:5001/api/instructor/dashboard/analytics', {
    ...
    const allRes = await fetch('http://localhost:5001/api/course');
    const allCourses = await allRes.json();
    
    const unownedCourse = allCourses.courses.find(c => c.price > 0 && !c.isEnrolled);
    if (!unownedCourse) {
    ...
    const histRes = await fetch('http://localhost:5001/api/payment/history', {
    ...
    const finalInstRes = await fetch('http://localhost:5001/api/instructor/dashboard/analytics', {
      headers: { 'Cookie': instCookie }
    });
...
setTimeout(testPayment, 3000); // Give server 3 seconds to start up
